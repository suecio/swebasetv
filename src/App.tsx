...
// --- MOCK DATA ---
// In the future, this will be fetched automatically from the YouTube API via your backend.
const MOCK_GAMES = [
  {
    id: 'game1',
...
export default function App() {
  const [currentView, setCurrentView] = useState('home'); // home, watch, multiview
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50 font-sans pb-20 md:pb-0 flex flex-col md:flex-row">
      ...
      <div className="md:hidden flex items-center justify-between p-4 bg-slate-900 border-b border-slate-800 sticky top-0 z-50">
        <div className="flex items-center gap-2 text-blue-500 font-bold text-xl">
          <MonitorPlay size={24} />
          <span>
            SweBase<span className="text-white">TV</span>
          </span>
        </div>
      ...
      <nav className="fixed bottom-0 w-full md:w-64 md:relative md:flex-shrink-0 bg-slate-900 border-t md:border-t-0 md:border-r border-slate-800 z-50 flex md:flex-col justify-around md:justify-start p-2 md:p-4 gap-2">
        ...
        <NavItem
          ...
          label="Archive"
          active={false}
          onClick={() => alert('Archive view coming soon!')}
        />
      </nav>
            ...
            <div className="p-4 md:p-8 flex-1">
              <div className="bg-slate-900 rounded-xl p-6 border border-slate-800">
                ...
                <p className="text-slate-400">
                  Match integration, live box scores, and team rosters could be
                  placed here in the future to create a complete second-screen
                  experience.
                </p>
              </div>
            </div>
          ...
          <div className="p-4 h-full flex flex-col animate-in fade-in">
            <div className="mb-4 flex justify-between items-center">
              ...
              <span className="text-sm bg-slate-800 px-3 py-1 rounded-full text-slate-300">
                Showing {liveGames.length} active streams
              </span>
            </div>

            {/* Grid adjusts based on number of live games. Assuming up to 4 for this layout */}
            <div
              className={`grid gap-2 md:gap-4 flex-1 min-h-[50vh] ${
                liveGames.length === 1
                  ? 'grid-cols-1'
                  : liveGames.length === 2
            ...
            >
              {liveGames.map((game, idx) => (
                <div
                  key={idx}
                  className="bg-black rounded-lg overflow-hidden relative border border-slate-800 aspect-video group"
                >
                  ...
                  <iframe
                    ...
                    src={`https://www.youtube.com/embed/${game.videoId}?mute=1`} // muted by default in multiview
                    title={game.title}
                    frameBorder="0"
                    allowFullScreen
                  ></iframe>
...
function GameCard({ game, onClick, isLarge = false }) {
  return (
    <div
      onClick={onClick}
    ...
    >
      ...
      >
        ...
        {/* Faux Baseball Background Pattern */}
      ...
      {/* Info Area */}
      <div className="p-4">
        <p className="text-xs text-blue-400 font-semibold tracking-wider mb-1">
          {game.league}
        </p>
        <h3
          className={`font-bold text-slate-100 leading-tight mb-2 ${
            isLarge ? 'text-xl' : 'text-base'
          }`}
        >
          {game.team1} <span className="text-slate-500 font-normal">vs</span>{' '}
          {game.team2}
        </h3>
